...
class Search_Filter_Admin_Notices {
	...
	public function set_all_post_notices_wlang($search_form_ids, $lang = "")
	{
		...
		{//the there is a form with WooCommerce
			...
			$plugins_array = apply_filters( 'active_plugins', get_option( 'active_plugins' ) );
			
			if ( !in_array( 'woocommerce/woocommerce.php', $plugins_array ) ) {
	...
	public function set_post_notices()
	{
		global $post;
		//setup errors
		$settings = (get_post_meta( $post->ID, '_search-filter-settings', true ));
		$fields = (get_post_meta( $post->ID, '_search-filter-fields', true ));
		...
		if(isset($settings['display_results_as']))
		{
			...
			else if($display_results_as=="custom_edd_store")
			{
				...
				if ( !in_array( 'easy-digital-downloads/easy-digital-downloads.php', $plugins_array ) ) {
