		...
		if(isset($this->form_settings['inherit_current_author_archive']))
		{
			if($this->form_settings['inherit_current_author_archive']=="1")
			{
				...
				if(is_author())
				{
					...
					$args['author'] = $term->ID; //here we set the post types that we want WP to search
				}
			}
		}
		...
		if($filter_value!="")
		{
			if($is_decimal==1)
			{
				$field_terms_results = $wpdb->get_results( 
					...
					FROM $this->table_name
					WHERE field_name = '$filter_name' 
						AND cast(field_value AS DECIMAL(12,2)) $compare_operator cast($filter_value as DECIMAL(12,2))
					"
				);
