...
class Search_Filter_Field_Reset extends Search_Filter_Field_Base {
	...
	public function get($field_data)
	{
		...
		{
			$returnvar .=  '<a href="#" class="search-filter-reset" data-search-form-id="'.$this->sfid.'">'.$values['label'].'</a>';
		}
		else
		{
			$returnvar .=  '<input type="submit" class="search-filter-reset" name="'.$field_name.'" value="'.$values['label'].'" data-search-form-id="'.$this->sfid.'">';
		}
