		...
		if(isset($args['meta_options']))
		{
			if(is_array($args['meta_options']))
			{
				...
				foreach ($args['meta_options'] as $meta_option)
				{
					...
					{	
						$option_count = $searchandfilter->get($this->sfid)->get_count_var($name, esc_attr($meta_option['option_value']));
					}
					else
					{
						$option_count = 0;
	...
	private function get_options_auto($args)
	{
		...
		if(($is_acf!=1)||(empty($acf_options)))
		{
			...
			foreach($field_options as $field_option)
			{
				if($this->auto_count==1)
				{	
					$option_count = $searchandfilter->get($this->sfid)->get_count_var($name, esc_attr($field_option->field_value));
				}
				else
				{
					$option_count = 0;
	...
	private function get_acf_options($args)
	{
		...
		if(isset($field['choices']))
		{
			...
			{			
				...
				{
					global $searchandfilter;
					$option_count = $searchandfilter->get($this->sfid)->get_count_var($name, esc_attr($value));
				}
				else
				{
					$option_count = 0;
